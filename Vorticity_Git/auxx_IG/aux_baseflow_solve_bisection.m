function  [psi,B] = aux_baseflow_solve_bisection(pa,ylin,Q,PLOT)

   


vort_fun = pa.vort_fun; H=pa.H; 
y = ylin; N=pa.N; dy=y(2)-y(1);
...
its=0;
%"Solving base-flow"
while (abs(error1)>10^(-7) & abs(error2)>10^(-7) & abs(error3)>10^(-7))   & its<100
    its=its+1;
