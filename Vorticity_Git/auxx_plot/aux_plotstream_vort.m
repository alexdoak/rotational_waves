function [] = aux_plotstream_vort(un,pa,de,fe,Nstream,X0_list,Y0_list,R)
...
plot(un.X(:,end),un.Y(:,end),'k','LineWidth',2);
plot(-un.X(:,end),un.Y(:,end),'k','LineWidth',2);
...
%{
contour(pa.wavelength+X,Y,Psi, Nstream,'k'); hold on;
contour(pa.wavelength+-X,Y,Psi,Nstream,'k');
plot(pa.wavelength+un.X(:,end),un.Y(:,end),'k','LineWidth',2);
plot(pa.wavelength+-un.X(:,end),un.Y(:,end),'k','LineWidth',2);
plot(pa.wavelength+un.X(:,1),un.Y(:,1),'k','LineWidth',2);
plot(pa.wavelength+-un.X(:,1),un.Y(:,1),'k','LineWidth',2);
%}
...
aux_plotstream_vort_critlayer(X0_list,Y0_list,R,pa,un,de)
